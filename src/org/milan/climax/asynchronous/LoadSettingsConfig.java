...
@SuppressLint("NewApi")
public class LoadSettingsConfig extends AsyncTask<String, Void, Void> {
	...
	public LoadSettingsConfig(android.app.Activity activity, Long profileID,
	...
	public LoadSettingsConfig(android.app.Activity activity, Long profileID,
		...
		String audioSongRequestStr = "{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"AudioLibrary.GetSongs\",\"params\":{\"properties\":[\"albumid\",\"duration\",\"artistid\",\"artist\",\"thumbnail\",\"playcount\"]}}";
		String audioArtistRequestStr = "{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"AudioLibrary.GetArtists\",\"params\":{\"properties\":[\"style\",\"description\",\"born\",\"yearsactive\",\"died\",\"thumbnail\",\"genre\"]}}";
		getAudioUpdateValues(mediaIp, audioRequestStr);
		getAudioSongUpdateValues(mediaIp, audioSongRequestStr);
		getAudioArtistUpdateValues(mediaIp, audioArtistRequestStr);
		getVideoMobileUpdateValues(mediaIp, videoRequestStr);
	}

	public LoadSettingsConfig() {

	}

	public LoadSettingsConfig(String requestStr) {
		String profile_ip_value = milanClimaxDataSource
				.CurrentProfileIp((long) profileID);
		// getRecentlyAddedVideoMobileUpdateValues(profile_ip_value,
		// requestStr);
	}
	...
	protected void onPostExecute(Void result) {
		...
		model.setCurrent_profile_ip_value(milanClimaxDataSource
				.CurrentProfileIp((long) profileID));
		Log.i(LOGCAT, "current_profileID--->" + profileID);
		String audioRequestArr[] = {
				"{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"AudioLibrary.GetAlbums\",\"params\":{\"properties\":[\"style\",\"albumlabel\",\"artistid\",\"type\",\"description\",\"thumbnail\",\"genre\",\"title\",\"artist\",\"rating\",\"year\"]}}",
				"{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"AudioLibrary.GetSongs\",\"params\":{\"properties\":[\"albumid\",\"duration\",\"artistid\",\"artist\",\"thumbnail\",\"playcount\"]}}",
				"{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"AudioLibrary.GetArtists\",\"params\":{\"properties\":[\"style\",\"description\",\"born\",\"yearsactive\",\"died\",\"thumbnail\",\"genre\"]}}" };

		String videoRequestStr = "{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"VideoLibrary.GetMovies\",\"params\":{\"properties\":[\"title\",\"director\",\"tagline\",\"fanart\",\"thumbnail\",\"genre\",\"setid\",\"rating\",\"year\",\"studio\",\"resume\",\"runtime\",\"plot\",\"trailer\"]}}";
		if (whichConfig.equalsIgnoreCase("audio")) {
			getAudioUpdateValues(profile_ip_value, audioRequestArr[0]);
			getAudioSongUpdateValues(profile_ip_value, audioRequestArr[1]);
			getAudioArtistUpdateValues(profile_ip_value, audioRequestArr[2]);
		} else if (whichConfig.equalsIgnoreCase("video")) {
			getVideoMobileUpdateValues(profile_ip_value, videoRequestStr);
		}
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (name.equalsIgnoreCase("result")) {
					...
					milanClimaxDataSource.bulkInsertAudioData(profileID,
			...
			reader.endObject();
			Toast.makeText(model.getContext(), "Successfully updated", 1000)
					.show();

		} catch (Exception e) {
			// TODO Auto-generated catch block
			milanClimaxDataSource.DeleteProfileDataIfExists(profileID);
			e.printStackTrace();
		}
	}
	...
	public void getAudioArtistUpdateValues(String profile_ip_value,
		...
		try {
			...
			reader.beginObject();
			String type = "artists";
			ArrayList<Integer> songArtistId = new ArrayList<Integer>();
			ArrayList<String> artistName = new ArrayList<String>();
			...
			while (reader.hasNext()) {
				...
				if (name.equalsIgnoreCase("result")) {
					...
					milanClimaxDataSource.bulkInsertAudioArtistsData(profileID,
							songArtistId, artistName, mediaIp, LabelArtist,
							ThumbnailArtist);
				} else {
					reader.skipValue();
				}
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (name.equalsIgnoreCase("result")) {
					...
					reader.endObject();
					milanClimaxDataSource.bulkInsertMovieData(profileID,
